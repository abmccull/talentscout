...
| FT-P0-02 | Critical | `shouldRouteToNegotiation` is defined at `src/engine/firstTeam/clubResponse.ts:480-487` but has no caller; `reportActions` can persist a `signed` response without opening a deal, while `initiateTransferNegotiation` can target any eligible player | The report, club decision, negotiation, and transfer are not one causal pipeline | Replace immediate `signed` with persisted committee authorization; require a valid decision/mandate before player-led negotiation |
...
| FT-P0-08 | High | `reportActions.ts:357-373` guarantees the first sufficiently convicted report at least an interested response; the tutorial copy at `src/components/game/tutorial/tutorialSteps.ts:314-327` says the club signed and a bonus was earned even though the trigger is merely the first positive response | The opening teaches that conviction manufactures success and can claim a transfer/bonus that never happened | Build a controlled but variable tutorial case whose positive aha is earned through evidence and persuasion; copy must reflect the actual state |
...
| FT-WP-19 | Current opening tutorial can say a player signed/bonus earned when only interest occurred and always teaches the same brief | Add a one-time, skippable/replayable opening-case director with at least four seeded football scenarios and earned outcomes | First five minutes teach interpretation; first hour delivers a memorable, variable First Team story | Existing: tutorial store/steps, NewGame, match/report flow. New: First Team opening director/content | Tutorial must teach without changing core probabilities | L | Four scenarios share teaching beats but vary club/role/candidates/conflict/outcome; copy matches state; experienced profile defaults off | Scenario determinism/divergence; tutorial persistence/replay; first-five-minute and first-hour E2E; moderated study | Player-experience migration only; preserve completed/skipped state |
