...
On a frozen final source run 20 seeds × 30 seasons with replay, reporting 1/5/10/20/30 boundaries. Run matched cautious/commercial/aggressive policies separately using only observable evidence; use focused adversarial regressions for exploits (there is no dedicated long-career exploit profile). Keep existing budgets intact. Retained-career native IndexedDB checkpoints must pass at completed seasons 1, 10 and 30, including exact commit read-back and a whole-browser-process restart. Interrupted recovery is tested separately; mocked soak saves certify none of these provider boundaries. The bounded consequence case must deliberately buy earned follow-up observations, file its chosen case and attempt an eligible pitch, recording every preparation predicate. An absence of attempts is a coverage failure, never evidence that recommendations succeed or fail correctly.
...
The `48315c3` acceptance run was stopped after both reference careers reached S5 with unacceptable lower-level roster shortages. Preserve those failures. Before the next full run, finish SYS-50–63 and combined checks, freeze a fresh source candidate, then run the bounded five-season roster/funding preflight. Only a healthy preflight can proceed to the complete acceptance run. Do not reuse old candidate checkpoints as final evidence. Native verification must separately establish receipt integrity, intentional migration behavior, exact committed state and a fresh browser-process reload; known gameplay-changing differences cannot be removed from its digest.
