...
On a frozen final source run 20 seeds × 30 seasons with replay, reporting 1/5/10/20/30 boundaries. Run cautious/commercial/aggressive and exploit policies separately using only observable evidence. Keep existing budgets intact. Real IndexedDB/file-provider checkpoints and interrupted recovery are separate from mocked soak saves.
