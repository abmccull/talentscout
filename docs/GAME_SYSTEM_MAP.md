...
| International football selection | Baseline alumni milestone checks ability/age and a seeded chance → international-call-up message | No national-team selection, international fixtures or authoritative call-up roster was found | Alumni/Inbox can imply more simulation than exists | Alumni milestones only (`youth/alumni.ts:360`) | Week / S / Partially Active abstraction; do not claim national-team simulation |
...
| Discovery/alumni memory: `career/discoveryTracking.ts`, `youth/alumni.ts`, `career/careerMoments.ts`, `chronology.ts`, `legacy*.ts` | Discovery, placement, observed career outcome and milestones → snapshots, retrospectives and legacy | Important player identity persists after exit; some alumni milestone rules are abstractions and must be labeled | Discoveries, Alumni, Career moments, Hall of Fame | Discovery/alumni records, chronology, moments and legacy | Action/week/season/retirement / D+S / Active with partial international abstraction |
...
- National-team selection/matches are not implemented as a full world subsystem; alumni international milestones are a limited abstraction.
