...
type InsightType = CulturalInsight["type"];
type InsightEffectAdjustment = Partial<Omit<FootballCultureInsightEffects, "version">>;
...
const DEFAULT_INSIGHT_ORDER = Object.freeze([
  "playingStyle",
  "developmentCulture",
  "mentalityPattern",
  "physicalTrait",
...
const ATTRIBUTE_DOMAINS = ["technical", "physical", "mental", "tactical", "hidden"] as const;
...
const FALLBACK_PLAYBOOK: FootballCulturePlaybook = Object.freeze({
  ...
  insightsByType: {
    playingStyle: insight(
      "playingStyle",
      "The local game has recurring tactical habits that change which actions appear often enough to trust.",
      "Understanding the local playing rhythm improves tactical interpretation.",
    ),
    developmentCulture: insight(
      "developmentCulture",
      "The player pathway is shaped by local coaching access, competition density, and registration structure.",
      "Understanding the pathway reduces development-context guesswork.",
    ),
    mentalityPattern: insight(
      "mentalityPattern",
      "Pressure, selection, and opportunity windows affect what behaviours a scout can really observe.",
      "Understanding the local pressure profile improves mental-context reads.",
    ),
    physicalTrait: insight(
      "physicalTrait",
      "The competition environment changes how early physical samples map to adult football utility.",
      "Understanding the sample environment improves physical interpretation.",
    ),
  },
  calendarWindows: [],
});
