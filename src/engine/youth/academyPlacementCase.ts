...
function mapBrief(
  ...
  // These judgment gates and the persisted snapshot come from one canonical
  // doctrine. The regional identity ranks opportunities but must not rewrite
  // the issued brief after its historical doctrine has been captured.
  const developmentPriority = source.developmentPriority;
  const riskTolerance = source.riskTolerance;
  const competitionPressure = clamp(
    ...
    + ((identity?.doctrine.sellingPressure ?? 50) - 50) * 0.12,
  );
  return {
    ...
    riskTolerance,
    weeklyWageBudget: source.weeklyWageBudget,
    competitionPressure,
