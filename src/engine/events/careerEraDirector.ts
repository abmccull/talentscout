...
import {
  CAREER_ERA_DEFINITIONS,
  CAREER_ERA_THEME_IDS,
  type CareerEraDefinition,
  type CareerEraTheme,
} from "./careerEraCatalog";
...
export type { CareerEraDefinition, CareerEraTheme } from "./careerEraCatalog";
...
const STORY_CANDIDATE_KINDS = new Set<StoryCandidateKind>([
  "standalone",
  "chain",
  "storyline",
  "special",
...
function validateCareerEraDefinition(
  definition: CareerEraDefinition,
  ...
  if (!CAREER_ERA_THEME_IDS.includes(definition.theme)) {
    issues.push({ path: "theme", message: "must be a supported career era theme" });
  }
  for (const [path, tokens, minimum] of [
    ["boostedCategoryTokens", definition.boostedCategoryTokens, 1],
    ["suppressedCategoryTokens", definition.suppressedCategoryTokens, 0],
  ] as const) {
    if (tokens.length < minimum || tokens.some((token) => !hasNonBlankString(token))) {
      issues.push({ path, message: `must contain at least ${minimum} non-empty token${minimum === 1 ? "" : "s"}` });
    }
    if (new Set(tokens).size !== tokens.length) {
      issues.push({ path, message: "must not contain duplicate tokens" });
    }
  }
  if (
    definition.boostedKinds.length === 0
    || definition.boostedKinds.some((kind) => !STORY_CANDIDATE_KINDS.has(kind))
  ) {
    issues.push({ path: "boostedKinds", message: "must contain supported story candidate kinds" });
  }
  if (new Set(definition.boostedKinds).size !== definition.boostedKinds.length) {
    issues.push({ path: "boostedKinds", message: "must not contain duplicate kinds" });
  }
  return issues;
}

export const CAREER_ERA_CONTENT_PACK = defineContentPack({
  manifest: {
    id: "talentscout.career-eras",
    kind: "career-era",
    schemaVersion: CONTENT_SCHEMA_VERSION,
    contentVersion: "career-eras.1",
  },
  entries: CAREER_ERA_THEME_IDS.map((theme) => CAREER_ERA_DEFINITIONS[theme]),
  getDefinitionId: (definition) => definition.theme,
  validateDefinition: validateCareerEraDefinition,
});

const ERA_DEFINITIONS: Readonly<Record<CareerEraTheme, CareerEraDefinition>> = Object.freeze(
  CAREER_ERA_CONTENT_PACK.byId as Record<CareerEraTheme, CareerEraDefinition>,
);

const THEMES = CAREER_ERA_THEME_IDS;
