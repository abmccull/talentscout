...
export function WeeklyStrategyPanel({
  ...
  const currentIntent = WEEKLY_INTENTS.find((intent) => intent.id === strategy.intentId);
  const currentPolicy = DELEGATION_POLICIES.find(
    ...
    <WorkspaceDisclosure
      ...
      summary={
        <div className="space-y-0.5">
          <p>{currentIntent?.shortLabel ?? "Balanced"} focus</p>
          <p>{currentPolicy?.label ?? "Desk lead decides"}</p>
        </div>
      }
      className="mb-4 border-violet-400/20 bg-[#10131a]/94 shadow-xl shadow-black/20"
      ...
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-3xl text-xs leading-relaxed text-zinc-300">
            ...
            Both standing orders persist until you change them.
          </p>
        </div>
        {latest && (
